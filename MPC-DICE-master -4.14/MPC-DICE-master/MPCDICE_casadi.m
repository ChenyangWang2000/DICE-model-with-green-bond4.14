...
import casadi.*
addpath('D:\Software\MatLab\casadi-windows-matlabR2016a-v3.5.5')
...
Data.state_list = {'time', 'T_AT', 'T_LO', 'M_AT', 'M_UP',...
                   'M_LO', 'K', 'B', 'sigma', 'L', 'A_TFP', 'Emission', 'C', ...
                   'mu', 's', 'tau', 'J'};               
Data.xMPC  = [];
Data.uMPC  = [];
Data.wMPC  = [];
...
switch Data.tf
    case 1 % Solve single OCP
        ...
        Data.lam_E = lambda(14,:); % dW/dE
        Data.lam_C = lambda(15,:)./Params.sc.C; % dW/dC
    ...
    otherwise %% Run MPC loop           
        ...
        for k = Data.t0 : Data.step : Data.tf
            ...
            u_opt = x_opt(end-3:end-1, [1:1:Data.step]); %actual input
            ...
            lambda  = full(res.lam_g);
            if k == Data.t0
                lambda = lambda(1:end-17);
                lambda = reshape(lambda, Data.nx, Data.N+1);
            else
                lambda = reshape(lambda, Data.nx, Data.N+1);
            end
            
            lambdaE = lambda(14,1:1:Data.step); %11
            lambdaC = lambda(15,1:1:Data.step)/Params.sc.C; %12
...
% Extended
B = Params.sc.B*Data.xMPC(8,:);

%% DICE "exogenous" states
sigma = Data.xMPC(9,:);
L = Params.sc.L*Data.xMPC(10,:);
A_TFP = Data.xMPC(11,:);

Emissions = Data.xMPC(14,:);  
Consumption = Params.sc.C*Data.xMPC(15,:);
...
for i=1:horizon
    ...
    Net_Output(i) = (1-tau(i))*Damages_Fraction(i)*...
